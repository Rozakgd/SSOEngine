...
if "%platform_choice%"=="3" (
    echo [INFO] Cleaning build and distribution directories...
    ...
    if exist "dist" (
        rmdir /s /q "dist"
        echo [INFO] Distribution directory cleaned.
    )
    
    if exist "raylib.zip" (
        del "raylib.zip"
        echo [INFO] Temporary raylib.zip removed.
    )
    
    if exist "error.log" (
        del "error.log"
        echo [INFO] Error log removed.
    )
    
    if exist "session.tmp" (
        del "session.tmp"
        echo [INFO] Session temp file removed.
    )
    
    if exist "resource.o" (
        del "resource.o"
        echo [INFO] Resource object file removed.
    )
    
    if exist "resource.res" (
        del "resource.res"
        echo [INFO] Resource file removed.
    )
    
    echo.
    echo [SUCCESS] Build and Dist directories have been purged.
    echo.
    ...
    echo [2] Close
    echo.
    set /p cleanup_choice="Choose [1/2]: "
...
call "!BUILD_TOOLS_PATH!\apksigner.bat" sign --ks "android\ssoengine.keystore" --ks-pass pass:rozak123 --v1-signing-enabled false --v2-signing-enabled true --v3-signing-enabled false --out "android\bin\SSOEngine.apk" "android\bin\SSOEngine.unsigned.apk"
if !errorlevel! neq 0 (
...
if "%mode_choice%"=="2" (
    ...
    set "CONSOLE_FLAG=-Wl,--subsystem,console"
...
if exist "%ICON_RES%" del "%ICON_RES%"
...
cmake -S . -B %BUILD_DIR% -G "MinGW Makefiles" ^
    ...
    -DCMAKE_EXE_LINKER_FLAGS="%CONSOLE_FLAG% -static -static-libgcc -static-libstdc++ -Wl,--gc-sections -Os" ^
    -DCMAKE_C_FLAGS="-Os -static" ^
    -DCMAKE_CXX_FLAGS="-Os -static -static-libgcc -static-libstdc++"
...
:cmake_error_menu
    echo.
    echo Choose action:
    echo [1] Copy Error to Clipboard
    echo [2] Return to Main Menu
    echo [3] Close
    echo.
    set /p cmake_error_choice="Choose [1/2/3]: "
    
    if "!cmake_error_choice!"=="1" (
        echo CMake Configuration Error > error.log
        echo Error Code: %errorlevel% >> error.log
        echo Timestamp: %date% %time% >> error.log
        type error.log | clip
        echo [SUCCESS] Error copied to clipboard!
        del error.log
        goto :cmake_error_menu
    )
    
    if "!cmake_error_choice!"=="2" (
        color
        cls
        goto :start_menu
    )
    
    if "!cmake_error_choice!"=="3" (
        color
        exit /b %errorlevel%
    )
...
:build_error_menu
    ...
    echo [3] Close
    echo.
    set /p build_error_choice="Choose [1/2/3]: "
    
    if "!build_error_choice!"=="1" (
        echo Windows Build Error > error.log
        echo Error Code: %errorlevel% >> error.log
        echo Timestamp: %date% %time% >> error.log
        type error.log | clip
        echo [SUCCESS] Error copied to clipboard!
        del error.log
        goto :build_error_menu
    )
    
    if "!build_error_choice!"=="2" (
        color
        cls
        goto :start_menu
    )
    
    if "!build_error_choice!"=="3" (
        color
        exit /b %errorlevel%
    )
...
move /Y "%OUTPUT_EXE%" "dist\" >nul
if %errorlevel% neq 0 (
    color 0C
    echo [ERROR] Failed to move EXE to dist folder
    echo Error Code: %errorlevel%
    echo Timestamp: %date% %time%
...
:dist_error_menu
    ...
    echo [3] Close
    echo.
    set /p dist_error_choice="Choose [1/2/3]: "
    
    if "!dist_error_choice!"=="1" (
        echo Distribution Move Error > error.log
        echo Error Code: %errorlevel% >> error.log
        echo Timestamp: %date% %time% >> error.log
        type error.log | clip
        echo [SUCCESS] Error copied to clipboard!
        del error.log
        goto :dist_error_menu
    )
    
    if "!dist_error_choice!"=="2" (
        color
        cls
        goto :start_menu
    )
    
    if "!dist_error_choice!"=="3" (
        color
        exit /b %errorlevel%
    )
...
if exist "build\assets.sso" (
    move /Y "build\assets.sso" "dist\" >nul
    if %errorlevel% neq 0 (
        color 0C
        echo [ERROR] Failed to move assets to dist folder
        echo Error Code: %errorlevel%
        echo Timestamp: %date% %time%
...
:assets_error_menu
        ...
        echo [3] Close
        echo.
        set /p assets_error_choice="Choose [1/2/3]: "
        
        if "!assets_error_choice!"=="1" (
            echo Assets Move Error > error.log
            echo Error Code: %errorlevel% >> error.log
            echo Timestamp: %date% %time% >> error.log
            type error.log | clip
            echo [SUCCESS] Error copied to clipboard!
            del error.log
            goto :assets_error_menu
        )
        
        if "!assets_error_choice!"=="2" (
            color
            cls
            goto :start_menu
        )
        
        if "!assets_error_choice!"=="3" (
            color
            exit /b %errorlevel%
        )
...
:zip_error_menu
    ...
    if "!zip_error_choice!"=="2" (
        color
        cls
        goto :start_menu
    )
    
    if "!zip_error_choice!"=="3" (
        color
        exit /b %errorlevel%
    )
...
if "%BUILD_MODE%"=="Debug" (
    echo.
    echo [INFO] Debug mode: Console will appear when you run .exe
    echo [INFO] You can see printf/cout output in console
)

echo.
echo [SUCCESS] Distribution package ready in dist/ folder
echo [INFO] Production archive: %ZIP_NAME%
echo [INFO] Build garbage cleaned up
echo.
